...
### 💎 极致美学
**Glassmorphism 2.0**  
采用磨砂玻璃设计语言，压缩字体做标题、等宽字体做数字，每一像素都经过精心雕琢。
...
- 🌑 **深色演播室**：`#0A0E13` 底色 + 三层 surface 拉开层次，光线偏暗也看得清。
- ⚡ **队伍配色**：玫红 `#FF2E63` 对电青 `#00E5FF`，全应用仅此两组高饱和色；`#D4FF3F` 专用于行动高亮。
- 🧊 **磨砂玻璃**：顶栏、底部弹层与 toast 使用 `backdrop-filter: blur(12px)`。
- 🔢 **数字优先**：标题用压缩字体，比分、计时与局分用等宽字体，读数不跳动。
...
- **存储**：LocalStorage，共 10 个键，全部留在本机
